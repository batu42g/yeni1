...
export default async function DashboardPage() {
    // Both of these runs on the server, leveraging parallel fetching if needed
    // getUserContext is cached per request
    const ctx = await getUserContext()
    const data = await getDashboardData()

    if (!ctx || !data) {
        // Fallback if session is invalid, layout should realistically catch this first
        return null
    }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* Staff Performance - Rendered on Client/Server boundary but fully populated */}
